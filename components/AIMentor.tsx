...
const AIMentor: React.FC = () => {
  ...
  const handleSend = async () => {
    ...
    setMessages(prev => [...prev, { role: 'model', content: advice || "I'm sorry, I couldn't process that." }]);
    setLoading(false);
  };

  return (
    <div className="max-w-4xl mx-auto h-[calc(100vh-12rem)] flex flex-col glass-card rounded-2xl shadow-xl border border-slate-200 overflow-hidden">
      {/* Header */}
      <div className="p-4 bg-indigo-600 text-white flex items-center gap-3">
        ...
        <div>
          <h3 className="font-bold">JEE Mentor</h3>
          <p className="text-xs opacity-80">AI Strategy & Doubt Solver</p>
        </div>
      </div>
      ...
      >
        {messages.length === 0 && (
          <div className="h-full flex flex-col items-center justify-center text-center space-y-4">
            ...
            <p className="text-slate-400 max-w-sm">
              Ask me about study techniques, hard physics concepts, or how to handle exam pressure.
            </p>
            <div className="flex flex-wrap gap-2 justify-center">
              {[
                ...
                <button 
                  key={q}
                  onClick={() => setInput(q)}
                  className="px-4 py-2 bg-slate-50 border border-slate-200 rounded-full text-sm text-slate-600 hover:bg-indigo-50 hover:border-indigo-200 transition-all"
                >
                  {q}
                </button>
              ))}
        ...
        {loading && (
          <div className="flex justify-start">
            <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 flex items-center gap-2">
      ...
      <div className="p-4 border-t border-slate-100">
        <div className="flex gap-2">
          <input
            type="text"
            placeholder="Ask your JEE mentor anything..."
            value={input}
